...
export function useCollaboration(files, onFilesChange) {
        ...
        if (restored) {
          ...
          if (storedFiles) {
            try {
              ...
              console.log(
                `📁 Aplicando ${fileCount} archivos restaurados desde localStorage`
              );
          ...
          if (restored.user.role !== "owner") {
            console.log(
              "👤 Usuario no es owner - solicitando archivos al owner..."
            );
            setTimeout(async () => {
  ...
  useEffect(() => {
    ...
    const handleFileChange = (payload) => {
      ...
      if (typeof payload.version === "number") {
        ...
        // Solo ignorar si la versión es MUCHO más antigua (diferencia > 5)
        ...
        // Aceptar versión igual o más reciente
        if (payload.version >= currentVersion) {
          fileVersionsRef.current[payload.filePath] = payload.version;
          console.log("✅ Versión aceptada:", payload.version);
        }
      } else {
        // Fallback a timestamp - ser más permisivo
        console.log("⏰ Timestamp incoming:", payload.timestamp, "vs last:", lastChangeTimestamp.current);
        const timeDiff = payload.timestamp - lastChangeTimestamp.current;
        // Solo ignorar si es más de 5 segundos más antiguo
        if (timeDiff < -5000) {
          console.log("⏸️ Timestamp MUY antiguo - ignorar (diff:", timeDiff, "ms)");
          return;
        }
      }
  ...
  const createSession = useCallback(
    async (sessionData) => {
      if (!isConfigured) {
        throw new Error(
          "Supabase no está configurado. Consulta la documentación para configurarlo."
        );
      }
